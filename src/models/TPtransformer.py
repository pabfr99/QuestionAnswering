...
class TPMultiHeadAttention(LightningModule):
    def __init__(self,
        ...
        # define linear layers for query, key, value, and role
        ...
        self.r_linear = nn.Linear(params.embed_size, params.embed_size, bias=True)
    ...
    def forward(self,
        ...
        r = r.view(batch_size, -1, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        # apply role vector to the output
        output = output * r
...
class EncoderLayer(LightningModule):
    def __init__(self,
        ...
        super().__init__()
        self.params = params
        self.attn = TPMultiHeadAttention(params)
...
class DecoderLayer(LightningModule):
    def __init__(self, params: HyperParams) -> None:
        ...
        self.params = params
        # self-attention
        self.self_attn = TPMultiHeadAttention(params)
        self.norm1 = nn.LayerNorm(params.embed_size)
        # cross-attention
        self.cross_attn = TPMultiHeadAttention(params)
        self.norm2 = nn.LayerNorm(params.embed_size)
        # feed-forward network
        self.ffn = nn.Sequential(
            nn.Linear(params.embed_size, params.hidden_size),
